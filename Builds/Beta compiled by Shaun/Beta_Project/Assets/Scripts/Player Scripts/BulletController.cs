...
public class BulletController : MonoBehaviour 
{
	...
	void OnTriggerEnter2D(Collider2D col)
	{
		...
		// Colliding with a wall destroys the bullet
		if(col.gameObject.tag == "CeilCol"
		   || col.gameObject.tag == "FloorCol"
		   || col.gameObject.tag == "DeathCol")
			Destroy(gameObject);

		// Colliding with a boss reduces the boss's health and destroys the bullet
		if (col.gameObject.tag == "Boss") {
			col.gameObject.GetComponent<BossEngine>().takeDamage(GameManager.instance.playerBulletDamage);
			Destroy(gameObject);
		}
	}
